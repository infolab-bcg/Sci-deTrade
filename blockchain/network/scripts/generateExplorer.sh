...
EOF
    ...
    for ((i=0; i<PEER_COUNT; i++)); do
        if [ $i -eq $((PEER_COUNT-1)) ]; then
            echo "                \"peer$i.org${org}.example.com\":{}" >> ${OUTPUT_FILE}
        else
            echo "                \"peer$i.org${org}.example.com\":{}," >> ${OUTPUT_FILE}
        fi
    done
    ...
    "organizations": {
        "Org${org}MSP": {
            "mspid": "Org${org}MSP",
            "adminPrivateKey": {
                "path": "/tmp/crypto/peerOrganizations/org${org}.example.com/users/Admin@org${org}.example.com/msp/keystore/priv_sk"   
            },
            "peers": [
EOF
...
EOF
    ...
    # 生成peer配置
    for ((i=0; i<PEER_COUNT; i++)); do
        PORT=${PEER_PORTS["$org,$i"]}
        echo "        \"peer$i.org${org}.example.com\": {" >> ${OUTPUT_FILE}
        echo "            \"tlsCACerts\": {" >> ${OUTPUT_FILE}
        echo "                \"path\": \"/tmp/crypto/peerOrganizations/org${org}.example.com/peers/peer$i.org${org}.example.com/tls/ca.crt\"" >> ${OUTPUT_FILE}
        echo "            }," >> ${OUTPUT_FILE}
        echo "            \"url\": \"grpcs://peer$i.org${org}.example.com:$PORT\"" >> ${OUTPUT_FILE}
        if [ $i -eq $((PEER_COUNT-1)) ] ; then
            echo "        }" >> ${OUTPUT_FILE}
        else
            echo "        }," >> ${OUTPUT_FILE}
...
echo "Explorer configuration files generated successfully for all organizations!"

# Add CONFIG_FILE variable
CONFIG_FILE="../../../explorer/config.json"
...
# Generate network configurations for each organization
for ((org=1; org<=ORG_COUNT; org++)); do
...
echo "Explorer configuration files and config.json generated successfully!"