...
import { initDatasetTable, deleteDatasetTable, addDataset, deleteDataset, getAllDatasets, getPublicDatasets, updateMaskingDatasetIPFSAddress } from './database/datasetTable.mjs';
...
export async function handleGetDatasetByName(req, res) {
    // TODO: 通过name查询数据集
}
...
export async function addDemoDatasets() {
    try {
        ...
        await addDataset('Physics', 'condensed_matter', '凝聚态物理材料数据', '固体材料的电子结构、磁性、超导性质测量数据，支持新材料设计和相变研究', 'demoDataOwner', true, false, false, false, await generateIPFSCID());
        ...
        await addDataset('Biology', 'ecology', '生态学监测数据', '生物多样性、种群动态、生态系统功能的长期监测数据，用于保护生物学和气候变化研究', 'demoDataOwner', true, false, false, false, await generateIPFSCID());
        ...
        await addDataset('Medicine', 'pharmacogenomics', '药物基因组学数据', '个体基因型与药物反应关系数据，用于精准医疗和个性化用药指导研究', 'demoDataOwner', true, false, false, false, await generateIPFSCID());
        ...
        await addDataset('ArtificialIntelligence', 'reinforcement_learning', '强化学习环境数据', '游戏、机器人控制、自动驾驶的环境交互数据，用于强化学习算法开发和评估', 'demoDataOwner', true, false, false, false, await generateIPFSCID());
        ...
        await addDataset('CyberSecurity', 'phishing_detection', '钓鱼攻击检测数据', '钓鱼邮件、恶意URL、社会工程攻击的样本数据，支持反钓鱼技术研究', 'demoDataOwner', true, false, false, false, await generateIPFSCID());
